...
class SettingsDialog(tk.Toplevel):
    def __init__(self, parent, current_work_min: int, current_break_min: int):
        ...
        # Work time
        ttk.Label(self, text="工作时间（分钟）:").grid(row=0, column=0, padx=10, pady=10, sticky="e")
        ...
        # Break time
        ttk.Label(self, text="休息时间（分钟）:").grid(row=1, column=0, padx=10, pady=10, sticky="e")
    ...
    def on_ok(self):
        self.result = (
            int(self.work_spin.get()) * 60,
            int(self.break_spin.get()) * 60
        )
        self.destroy()
